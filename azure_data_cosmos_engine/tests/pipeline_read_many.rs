...
pub fn read_many() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let expected_even_first = vec![
        ...
        EngineResult {
            items: vec![
                // All even partition items first
    ...
    let expected_odd_first = vec![
        EngineResult {
            items: vec![],
            requests: vec![
                DataRequest::with_query(0, "odd", None, expected_odd_query.to_string(), true),
    ...
    assert!(
        results == expected_even_first || results == expected_odd_first,
        "Results must match either even-first or odd-first ordering.\nActual: {:#?}",
        results
    );
